#Função SCHEFFE para teste de médias de Scheffé

using DataFrames, Statistics, PyPlot, Distributions

function Scheffe(Tratamento,Repeticao,Variavel,GLResiduo,QMResiduo,Alpha)
    X=DataFrame()
    X.Trat=Tratamento
    X.Rep=Repeticao
    X.Y=Variavel
    ...
    F=quantile(FDist(I-1, GLResiduo),(1-Alpha))
    DMS=sqrt((I-1)*F)*sqrt(2*(QMResiduo/J))
...
#Scheffe(Tratamento,Repeticao,Variavel,GLResiduo,QMResiduo,Alpha)